...
test('the public site exposes reproducible Python and JavaScript installation paths', async () => {
  ...
  for (const page of [home, quickstart, download, llms]) {
    assert.match(page, /python -m pip install govp==0\.1\.10/);
    assert.match(page, /npm install @govp\/verifier@0\.1\.7/);
  }
  ...
  assert.match(download, /https:\/\/www\.npmjs\.com\/package\/@govp\/verifier\/v\/0\.1\.7/);
  assert.match(download, /https:\/\/github\.com\/govp-protocol\/govp-js\/releases\/tag\/v0\.1\.7/);
  assert.match(download, /https:\/\/pypi\.org\/project\/govp\/0\.1\.10\//);
